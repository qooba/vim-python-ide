...
* Must install a patched font that contains required glyphs: 
    * on host computer: 
```
   curl -L -O https://github.com/ryanoasis/nerd-fonts/releases/download/v2.1.0/Mononoki.zip && unzip Mononoki.zip -d Mononoki && cp -r Mononoki /usr/share/fonts/truetype/ && fc-cache -f -v
```
